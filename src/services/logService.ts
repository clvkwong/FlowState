...
function docToLog(id: string, data: Record<string, unknown>): HabitLog {
  const loggedAt = data.loggedAt as { toDate?: () => Date } | string | undefined;
  return {
    id,
...
export async function upsertLog(input: UpsertLogInput): Promise<HabitLog> {
  ...
  if (input.completedTasks !== undefined) payload.completedTasks = input.completedTasks;

  if (!snapshot.empty) {
    const existing = snapshot.docs[0];
    await updateDoc(doc(db, 'habitLogs', existing.id), payload);
    return docToLog(existing.id, { ...existing.data(), ...payload, loggedAt: new Date().toISOString() });
  }
...
export async function deleteLogsForHabit(userId: string, habitId: string): Promise<void> {
  const q = query(
    collection(db, 'habitLogs'),
    where('userId', '==', userId),
  ...
  const snapshot = await getDocs(q);
  await Promise.all(snapshot.docs.map((d) => deleteDoc(doc(db, 'habitLogs', d.id))));
}
